...
static void	load_weapon_model(t_hunt_stats *out)
{
	...
	selected[0] = '\0';
	if (weapon_selected_load(tm_path_weapon_selected(),
	...
	/*
	 * Remplit les champs détaillés attendus par tracker_view :
...
/*
 * Par défaut, on charge markup.ini à chaque compute (simple et safe).
 * Si plus tard tu veux optimiser, on pourra mettre un cache global + reload.
 */
static int	load_markup_db(t_markup_db *mu)
{
	...
	{
		/* fallback: pas de markup.ini => MU = TT */
		markup_db_free(mu);
		markup_db_init(mu);
		return (-1);
	}
...
static double	apply_markup_value(const t_markup_db *mu, const char *item_name, double tt_value)
{
	t_markup_rule	r;
...
/*
 * Optionnel: si tu ajoutes des champs dans t_hunt_stats,
 * tu peux activer ça en ajoutant un define dans tracker_stats.h :
...
int	tracker_stats_compute(const char *csv_path, long start_line, t_hunt_stats *out)
{
	...
	/* Load markup.ini (fallback safe if missing) */
	(void)load_markup_db(&mu);
	...
	while (fgets(buf, (int)sizeof(buf), f))
	{
		...
		/* CSV: ts, type, name, qty, value, raw */
		...
		/* value extraction */
		{
			...
			/* Détection expense:
			 * - on ajoute des matches EXACTS (robuste)
			 * - puis on garde ton heuristic icontains (sans rien enlever)
			 */
			int	is_expense_exact = (strcmp(type, "SPEND") == 0)
			|| (strcmp(type, "DECAY") == 0)
			|| (strcmp(type, "AMMO") == 0)
			|| (strcmp(type, "REPAIR") == 0);
			
			int	is_expense = is_expense_exact
			|| str_icontains(type, "SPEND")
			...
			/*
			 * IMPORTANT:
			 * - On garde loot_ped = TT loot (comme avant).
			 * - On applique le markup sur LOOT/RECEIVED uniquement
			 *   (name = item name). Ça n’enlève rien, ça ajoute juste une info
			 *   optionnelle via TM_STATS_HAS_MARKUP.
			 */
			if (has_value && (is_loot || (!is_expense && v > 0.0)))
			{
				...
				if (is_loot)
				{
					double	final = apply_markup_value(&mu, name, v);
	...
	/* MODIF #1: ne plus sous-estimer les depenses
	 * Si on a des depenses loggées ET un modèle arme, on prend le MAX des deux.
	 */
	out->expense_used_is_logged = (out->expense_events > 0);
	
	if (out->expense_used_is_logged && out->has_weapon && out->shots > 0)
	{
		out->expense_used = (out->expense_ped_logged > out->expense_ped_calc)
		? out->expense_ped_logged : out->expense_ped_calc;
	}
