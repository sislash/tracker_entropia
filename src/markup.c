...
static int	db_reserve(t_markup_db *db, size_t need)
{
    ...
    while (new_cap < need)
        new_cap *= 2;
    new_items = (t_markup_rule *)realloc(db->items, new_cap * sizeof(t_markup_rule));
    if (!new_items)
        return (-1);
    db->items = new_items;
    db->cap = new_cap;
...
int	markup_db_get(const t_markup_db *db, const char *item_name, t_markup_rule *out)
{
    if (!db || !item_name || !out)
        return (0);
    for (size_t i = 0; i < db->count; ++i)
...
/*
 * * Called by parser when it wants to insert:
