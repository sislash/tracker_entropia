...
static void	print_header_fileinfo(const t_hunt_stats *s)
{
	print_status_line("Lecture du fichier",
...
static void	print_header_pageinfo(void)
{
	print_status_linef("PAGE", "%d/%d  (1-4 direct, A/D changer)",
...
static void	print_header(const t_hunt_stats *s)
{
	...
	print_hrs();
	print_header_fileinfo(s);
	print_hrs();
	print_header_pageinfo();
}
...
static void	page_resume_activity(const t_hunt_stats *s)
{
...
static void	page_resume(const t_hunt_stats *s)
{
	...
	page_resume_activity(s);
}

static void	page_loot_sweat(const t_hunt_stats *s)
{
	section_begin("SWEAT");
	print_status_linef("Extractions / Total", "%ld / %ld",
					   s->sweat_events, s->sweat_total);
	print_ped_pec("Valeur (PED)",
				   (double)s->sweat_total * (double)EU_SWEAT_PED_PER_BOTTLE, 4);
	print_status_linef("Prix sweat", "%.4f PED / bouteille (%.1f / 1000)",
				   (double)EU_SWEAT_PED_PER_BOTTLE,
				   (double)EU_SWEAT_PED_PER_BOTTLE * 1000.0);
	if (s->sweat_events > 0)
	{
...
static void	page_loot_markup(const t_hunt_stats *s)
{
	...
	print_ratio_pct_line("Return (TT+MU)", s->loot_total_mu_ped,
						 s->expense_used);
...
static void	page_expenses_model_details(const t_hunt_stats *s)
{
...
static void	page_expenses(const t_hunt_stats *s)
{
	...
	if (s->has_weapon && s->shots > 0)
		page_expenses_model_details(s);
}

#ifdef TM_STATS_HAS_MARKUP
static void	page_results_markup(const t_hunt_stats *s)
{
...
static void	page_results_top_mobs(const t_hunt_stats *s)
{
	size_t	i;
	...
	{
		print_status_linef("", "%2zu) %-28s %ld", i + 1,
...
static void	page_results(const t_hunt_stats *s)
{
	...
	#ifdef TM_STATS_HAS_MARKUP
	page_results_markup(s);
	#endif
	page_results_top_mobs(s);
}

static void	page_dispatch(const t_hunt_stats *s)
{
